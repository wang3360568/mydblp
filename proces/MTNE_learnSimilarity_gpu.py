...
class MTNE_learnSimilarity_nocompany():
    ...
    def lossfuction(self,X,W,A,F,D):
        ...
        part3=0.5*(self.lamda*self.norm(W)+self.gamma*self.norm(D))
    ...
    def loss(self,N,M,F,isnotdense):
        if not isnotdense:
            return self.norm(N-np.dot(M,F))
        else:
            newN=np.dot(M,F)
            return self.norm(N-self.getMask(N)*newN)
    ...
    def norm(self,first):
        return math.sqrt(np.sum(first*first))
