...
class MTNE_learnSimilarity_nocompany():
    ...
    def MTNE(self):
        ...
        while abs(loss_t-loss_t1) >= self.epsilon:
            ...
            for key in self.edgeDict:
                ...
                W=W-nita*(self.rho*lw_p2+self.lamda*lw_p4+self.zeta*W)
                ...
                P=P-nita*(lp_p2+self.rho*lp_p4+self.zeta*P)
                P=self.chechnegtive(P,None,None)

                P_list[counter]=P
                M_list[counter]=Mprime

                loss_t1_part=self.lossfuction(Y,Mprime,P,M,W)
    ...
    def lossfuction(self,Y,Mprime,P,M,W):
...
if __name__ == "__main__":
    ...
    p_list=mtne.MTNE()
    pickle.dump(p_list, open('P_list_sharedSpace_'+label+'.dat', "wb"), True)