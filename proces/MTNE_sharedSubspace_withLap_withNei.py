...
class MTNE_learnSimilarity_nocompany():
    ...
    def MTNE(self):
        ...
        for key in self.edgeDict:
            ...
            neiDict=self.getNeiborghood(index,key)
            neiDict_list.append(neiDict)
        ...
        while abs(loss_t-loss_t1) >= self.epsilon:
            ...
            for key in self.edgeDict:
                ...
                if neiDict!=None:
                    deltaP=P.copy()
                    lastP=P_list[counter-1]
                    for keyId in neiDict:
                        thep_last=lastP[neiDict[keyId]]
                        deltaP[keyId]=thep_last
                    lp_p6=P-deltaP
                    P=P-nita*(lp_p2+self.rho*lp_p4+self.theta*lp_p5+self.alpha*lp_p6)
                ...
                if neiDict!=None:
                    loss_t1_part=self.lossfuction(Y,Mprime,P,M,W,L,deltaP)
                else:
    ...
    def lossfuction(self,Y,Mprime,P,M,W,L,P_last=None):
        ...
        if P_last is not None:
            part4=0.5*(self.alpha*np.linalg.norm(P-P_last))
    ...
    def getNeiborghood(self,i,key):
        if i==0:
            ...
            for keyId in current:
                if keyId in last:
                    indexDict[index]=last.index(keyId)
                index+=1
            return indexDict
...
if __name__ == "__main__":
    ...
    p_list=mtne.MTNE()
    pickle.dump(p_list, open('P_list_sharedSpace_withLap_withNei_'+label+'.dat', "wb"), True)